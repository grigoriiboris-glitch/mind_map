...
import (
	...
	"strings"
	"time"
	"github.com/mymindmap/api/pkg/core/utils/ratelimiter"
...
type AuthService struct {
	...
	rateLimiter *ratelimiter.RateLimiter// Лимитер запросов (опционально)
}
...
func NewAuthService(userRepo UserRepositoryInterface, config *Config) (*AuthService, error) {
	...
	if config.EnableRateLimit {
		service.rateLimiter = ratelimiter.NewRateLimiter(
...
func (s *AuthService) RegisterUser(ctx context.Context, req *userreq.Register) (*models.User, error) {
...
func (s *AuthService) LoginUser(ctx context.Context, req *userreq.Login) (*TokenPair, error) {
    ...
    //log.Println("GetUserByEmail: user %v", user.Password)
	if user == nil {
